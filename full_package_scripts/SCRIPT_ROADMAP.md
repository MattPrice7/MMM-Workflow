...
6. Future model extensions / maybe
   - Done: add optional context-varying effectiveness modifiers directly to the Stan workflow for named hypotheses. Inputs can come from metadata tuple cells or a `context_effects` table; drivers are train-standardized; sign constraints are enforced in Stan; decomposition and optimizer helpers use the same multiplier path. The `time` key exists only for explicit advanced drift hypotheses and is not a standard recommendation.
