...
1. `hier_mmm.R` / `hier_mmm.stan`
   ...
   - Done: add metadata guardrails for group coefficient pooling through `coef_hierarchy_scope = auto/global/none/keyed`, `hierarchy_key`, `model_id_parts`, and `hierarchy_part_indices`. `auto` uses group coefficient hierarchy only for media/reach-frequency treatment roles by default, matching Meridian's separation of media treatments from controls/base. `global` explicitly uses all-group pooling; `none` blocks hierarchy for the variable; `keyed` activates a single-level keyed pooling family when the one-time `coef_hierarchy_part_indices` input or metadata `hierarchy_part_indices` can map arbitrary composite `group_col` values such as `region_retailer_product` into families such as product or region-product. Indices are R-style one-based by default, so product is `3` in `region_retailer_product`. Per-variable metadata can override the global key. If a variable's key produces only one family, Stan collapses it to the regular global hierarchy.
   ...
   - Future: implement optional multi-level nested/crossed hierarchy only after single-level keyed pooling is stable. Keep `group_col` generic and avoid hardcoded names like DMA/product/LOB. `source_entity` should remain source/halo metadata, not the main hierarchy-family key.
...
6. Future model extensions / maybe
   - Done: add optional context-varying effectiveness modifiers directly to the Stan workflow for named hypotheses. Inputs can come from metadata tuple cells or a `context_effects` table; real data-column drivers are train-standardized; sign constraints are enforced in Stan; decomposition and optimizer helpers use the same multiplier path. Self-context is blocked, plain `time` context is blocked by default, and context prior/posterior diagnostics include risk/review flags.
   ...
   - Future maybe: add a true AR/random-walk time-varying multiplier only if there is a strong use case and the current context-key approach is insufficient.
