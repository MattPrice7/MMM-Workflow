...
- `rolling_latest_core_keyed_coefficient_hierarchy`
  - Current rolling state after promoting `coef_hierarchy_scope = "keyed"` from metadata-only to an active single-level Stan coefficient pooling option. Analysts can use arbitrary composite `group_col` / model IDs such as `region_retailer_product` and pass one-time `coef_hierarchy_part_indices = 3` to pool within product, or `coef_hierarchy_part_indices = c(1, 3)` to pool within region-product. Indices are R-style one-based by default, with an optional zero-based escape hatch. The implementation supports one keyed hierarchy definition per model run, keeps curve parameters shared at variable level, keeps `source_entity` as source/halo metadata rather than a hierarchy key, collapses a one-family keyed map to the regular global hierarchy to avoid an unnecessary extra latent layer, and leaves true multi-level nested/crossed hierarchy as future work. Validated with Stan source, 43 Stan contract checks, Stan compile/context smoke, and a tiny active-keyed Stan fit smoke.
