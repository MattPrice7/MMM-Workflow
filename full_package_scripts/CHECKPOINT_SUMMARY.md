...
- `rolling_latest_core_context_effect_modifiers`
  - Current rolling state after adding opt-in context-varying effectiveness modifiers directly to the Stan workflow. Analysts can define context effects in metadata with tuple syntax such as `(context_col, 0.02, 0.03, +)`, or pass a `context_effects` table to `fit_hier_mmm()` / `prepare_stan_data_hier_mmm()`. Context drivers are standardized on training rows only; positive/negative/free signs map to constrained Stan parameter blocks; and the same multiplier is used by fitted transforms, decomposition, ROI/mROI, and optimizer contribution helpers. The special `time` key is supported only for explicit advanced drift hypotheses and is not a recommended default. This remains off by default and is intended for named hypotheses, not free time-varying coefficients. Validated with Stan source, workflow source, Stan compile, 39 Stan contract checks, 23 transform checks, 39 optimizer checks, and a 7-check known-truth context smoke test where TV context raised search effectiveness and macro context raised price sensitivity magnitude. The price-context test recovered direction but overestimated effect size, so context modifiers should be reviewed through the prior/posterior context audit rather than treated as automatic truth.
