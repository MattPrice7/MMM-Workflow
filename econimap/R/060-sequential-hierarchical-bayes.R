...
econ_seq_fit_root_pooled_positive_nlme <- function(design,
                                                   root_data,
                                                   root_curve_spec) {
  ...
  media_idx <- match("total_paid_media_scaled", original_names)
  if (is.na(media_idx)) return(NULL)
  baseline_idx <- setdiff(seq_len(ncol(X)), media_idx)
  base_beta <- tryCatch(qr.solve(X, design$y), error = function(e) rep(NA_real_, ncol(X)))
  media_start <- base_beta[media_idx]
  if (!is.finite(media_start) || media_start <= 1e-8) return(NULL)
  parameter_names <- c(paste0("root_base_beta_", seq_along(baseline_idx)), "root_log_media_beta")
  base_terms <- paste0(parameter_names[seq_along(baseline_idx)], " * ", safe_names[baseline_idx])
  model_formula <- stats::as.formula(paste(
    "root_y__ ~", paste(c(base_terms, paste0("exp(root_log_media_beta) * ", safe_names[media_idx])), collapse = " + ")
  ))
  fixed_formula <- stats::as.formula(paste(paste(parameter_names, collapse = " + "), "~ 1"))
  fit_data <- as.data.frame(X)
  fit_data$root_y__ <- design$y
  fit_data$root_group__ <- factor(groups)
  start <- c(base_beta[baseline_idx], log(media_start))
  names(start) <- parameter_names
  mixed_fit <- tryCatch(
    nlme::nlme(
      model = model_formula,
      fixed = fixed_formula,
      random = stats::as.formula("root_log_media_beta ~ 1"),
      groups = ~root_group__,
      data = fit_data,
      start = start,
      na.action = stats::na.fail,
      method = "ML",
      control = nlme::nlmeControl(returnObject = TRUE, msMaxIter = 100L, pnlsMaxIter = 20L, msVerbose = FALSE)
    ),
    error = function(e) NULL
  )
  if (is.null(mixed_fit)) return(NULL)
  fixed_beta <- nlme::fixef(mixed_fit)
  if (!all(parameter_names %in% names(fixed_beta))) return(NULL)
  group_coef <- tryCatch(stats::coef(mixed_fit), error = function(e) NULL)
  if (is.null(group_coef) || !("root_log_media_beta" %in% names(group_coef))) return(NULL)
  log_beta_by_group <- setNames(as.numeric(group_coef[["root_log_media_beta"]]), rownames(group_coef))
  log_beta_by_row <- unname(log_beta_by_group[groups])
  if (any(!is.finite(log_beta_by_row))) return(NULL)
  beta_by_row <- exp(log_beta_by_row)
  beta_common <- exp(as.numeric(fixed_beta[["root_log_media_beta"]]))
  baseline_fit <- if (length(baseline_idx)) {
    as.numeric(X[, baseline_idx, drop = FALSE] %*% as.numeric(fixed_beta[parameter_names[seq_along(baseline_idx)]]))
  } else rep(0, nrow(X))
  fitted <- baseline_fit + beta_by_row * X[, media_idx]
  ...
  fixed_log_sd <- if ("root_log_media_beta" %in% rownames(fixed_cov)) {
    sqrt(pmax(fixed_cov["root_log_media_beta", "root_log_media_beta"], 0))
  } else NA_real_
  random_variance <- tryCatch(
    suppressWarnings(as.numeric(nlme::VarCorr(mixed_fit)[1, "Variance"])),
    error = function(e) NA_real_
  )
  group_weight <- tapply(weighted_feature, groups, sum) / spend_total
  group_beta <- exp(log_beta_by_group[names(group_weight)])
  fixed_sd_component <- if (is.finite(fixed_log_sd)) abs(effect) * fixed_log_sd else 0
  random_sd_component <- if (is.finite(random_variance) && random_variance >= 0) {
    sqrt(random_variance * sum((as.numeric(group_weight) * group_beta)^2))
  } else 0
  effect_sd <- sqrt(fixed_sd_component^2 + random_sd_component^2)
  ...
  slopes <- data.table::data.table(
    root_group__ = names(log_beta_by_group),
    root_media_beta = exp(as.numeric(log_beta_by_group)),
    root_media_beta_deviation = exp(as.numeric(log_beta_by_group)) - beta_common,
  ...
  out <- data.table::data.table(
    ...
    root_effect_sign = "positive",
    root_sign_boundary_active = FALSE,
    root_fit_method = "frequentist_positive_log_mixed_effects_profile_ml",
    root_geo_media_effect_mode = "partially_pooled_log_normal",
    root_geo_media_effect_group_n = data.table::uniqueN(groups),
    root_geo_media_effect_tau = sqrt(pmax(random_variance, 0)),
    root_geo_media_effect_scale = "log",
...
econ_seq_fit_root_pooled_lme <- function(design,
  ...
  # A normal random-slope approximation can cross zero even when the root is
  # explicitly positive. Do not export economically impossible geo effects as
  # evidence. A positive log-scale mixed-effects root is a separate model; the
  # current linear mixed approximation must fall back to the constrained shared
  # effect until that implementation is available.
  if (identical(root_effect_sign, "positive") && any(beta_common + random_by_group <= 0)) return(NULL)
  if (identical(root_effect_sign, "negative") && any(beta_common + random_by_group >= 0)) return(NULL)
  ...
  out <- data.table::data.table(
    ...
    root_geo_media_effect_group_n = data.table::uniqueN(groups),
    root_geo_media_effect_tau = sqrt(pmax(random_variance, 0)),
    root_geo_media_effect_scale = "raw",
    root_geo_media_effect_fallback_reason = NA_character_,
    root_time_baseline = design$root_time_baseline,
    root_time_basis_n = design$root_time_basis_n,
    root_time_basis_penalty_applied = FALSE
...
econ_seq_fit_root_lm <- function(root_data,
  ...
  if (identical(root_geo_media_effect, "partially_pooled") && !prior$active) {
    pooled <- if (identical(root_effect_sign, "positive")) {
      econ_seq_fit_root_pooled_positive_nlme(
        design = design,
        root_data = root_data,
        root_curve_spec = design$media$curve_spec
      )
    } else {
      econ_seq_fit_root_pooled_lme(
        design = design,
        root_data = root_data,
        root_effect_sign = root_effect_sign,
        root_curve_spec = design$media$curve_spec
  ...
  out <- data.table::data.table(
    ...
    root_geo_media_effect_scale = NA_character_,
    root_geo_media_effect_fallback_reason = if (identical(root_geo_media_effect, "partially_pooled")) {
      if (prior$active) "root_effect_prior_requires_shared_profile_fit" else if (identical(root_effect_sign, "positive")) {
        "positive_log_scale_mixed_effects_fit_unavailable_or_failed"
      } else "pooled_normal_random_slopes_failed_or_were_unavailable"
    } else NA_character_,
    root_time_baseline = design$root_time_baseline,
    root_time_basis_n = design$root_time_basis_n,
...
econ_seq_block_bootstrap_root <- function(root_data,
  ...
  econ_seq_with_seed(seed, {
    rows <- lapply(seq_len(reps), function(ii) {
      ...
      boot_y_model <- base_fitted + residual_sample$residuals
      if ("root_y_model__" %in% names(boot)) {
        # The bootstrap fit uses the model-scale KPI. When the root is
        # per-capita scaled, resampling only root_y__ leaves root_y_model__
        # unchanged and silently produces identical bootstrap replicates.
        boot[, root_y_model__ := boot_y_model]
        if ("root_outcome_multiplier__" %in% names(boot)) {
          boot[, root_y__ := root_y_model__ * root_outcome_multiplier__]
        } else {
          boot[, root_y__ := root_y_model__]
        }
      } else {
        boot[, root_y__ := boot_y_model]
      }
