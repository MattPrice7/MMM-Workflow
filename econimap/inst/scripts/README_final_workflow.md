...
- `bau_response_curves.R`
  ...
  - When a KPI is supplied, BAU runs an observational coarse-to-fine curve search. Retention is profiled first at the anchor using rolling blocked validation; saturation and shape are then searched only within the statistically competitive retention set so carryover and saturation cannot freely substitute for one another. The one-standard-error rule chooses the candidate closest to the supplied/default anchor whenever several curves fit similarly. With geo and population data, search evidence uses per-capita support and KPI; without population it uses group mean indexing. These are observational updates, not causal calibration.
  - Returns the anchored curve, selected regularized curve, full candidate profile, search diagnostics, and q05/q50/q95 one-standard-error sensitivity envelopes. The envelopes are explicitly labeled as observational sensitivity ranges rather than Bayesian credible intervals. Missing KPI, insufficient active periods, weak support variation, or unstable lag evidence retains the anchored curve.
...
- `hier_mmm.R`
  ...
  - Supports first-class `role = "reach_frequency"` rows using `reach_col`, `frequency_col`, and optional `population_col`. Reach is population-scaled and linear; frequency uses a learned Hill response; the resulting effective-media signal is adstocked. These rows always use Hill because Weibull-over-combined-support is not the reach/frequency model.
  - Returns `frequency_response_curves` and `frequency_response_curves.csv` for RF variables. Planning varies average frequency while holding approximate impressions (`reach x frequency`) constant and should be reconciled to platform delivery constraints before activation.
  ...
  - Accepts `calibration_input` directly in `fit_hier_mmm()`. External lift, ROI plus incremental spend, or cost-per-KPI plus incremental spend is converted to raw-KPI incremental lift and enters a joint normal calibration likelihood with required SD/precision. `experiment_calibration_diagnostics` reports posterior lift and prior/evidence tension after fitting.
...
- Quasi-geo test results can also be converted into Stan coefficient-prior metadata with `qgt_build_stan_prior_handoff()` and `qgt_apply_stan_prior_handoff()`. The handoff uses only channel-specific usable evidence for individual coefficients and keeps bundle shocks as bundle evidence. When a channel has usable uncertainty-bounded marginal-response evidence at multiple support levels, the script fits a conservative Hill dose-response read, shrinks its median-support saturation anchor toward 50%, and carries the anchor plus its SD/precision into Stan. Insufficient or confounded dose evidence remains diagnostic and does not create a curve prior.
- Linear non-media treatments use `non_media_baseline_values`; the Meridian-aligned default is `min`, with `max`, `mean`, `median`, `zero`, and custom numeric references available. Controls use `control_reference_values = "mean"` by default for a noncausal reporting contrast. Non-media and controls are z-scored using training rows only. Business-prior inputs can be supplied as `coefficient`, `contribution`, `elasticity`, or `effect_per_sd`; conversion preserves the analyst's SD or inverse-variance precision in the audit while mapping it into Stan coefficient space.
