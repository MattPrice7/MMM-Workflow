...
- `rolling_latest_production_geometry_probe`
  - Fixed an auto coefficient-parameterization failure when a holdout is present: per-group training counts now use an explicit logical holdout filter. Added regression coverage. The sequential validation harness now has an opt-in one-chain focused profile that uses production-facing sampler choices rather than the deliberately harsh smoke configuration. On the clean six-geo direct baseline, 300 warmup plus 100 sampling iterations with diagonal metric, Pathfinder initialization, and auto coefficient parameterization reduced tree-depth hits from the smoke run's 100% to 1%, but still produced 8 divergences and low E-BFMI. This is one-chain geometry evidence only, not a convergence or sequential-recovery verdict. Next: isolate the remaining funnel with direct fixed-curve, no-coefficient-hierarchy, and alternate centered/non-centered ablations before resuming staged comparisons.
...
- `rolling_latest_simmmulator_calibration_validation`
  - Added an opt-in Meta `siMMMulator` model-level benchmark. Meta's simulator supplies realistic baseline, flighting, spend, delivery, cost noise, and a native ROI audit; the test overlays exact Econimap Hill/Weibull truth so curve and attribution recovery have a precise target. The benchmark verifies exact IKPC-to-Stan coefficient conversion, native joint experiment calibration, decomposition reconciliation, fit, known-truth contribution/ROI recovery, and sampler geometry. Final result: 8/8 checks passed, train R2 0.9579, worst channel contribution/ROI relative error 31.3%, zero divergences, zero treedepth hits, and minimum BFMI 0.4455. A broad 20% coefficient-scale prior alone did not identify TV on the correlated panel; native calibration was required, which is retained as an explicit identification lesson rather than hidden by easier synthetic data. The default aggregate bundle suite also passed in full, and the built `econimap_0.1.0.tar.gz` completed `R CMD check --no-manual` with `Status: OK`.

- `rolling_latest_core_nonmedia_quasi_curve_cv_multimarket`
  - Added training-only non-media/control z-scoring and explicit counterfactual/reference values; contribution, elasticity, standardized-effect, and coefficient priors now convert auditably into Stan coefficient space while preserving SD/inverse-variance precision. Quasi-geo now emits uncertainty-weighted curve evidence points, conservative multiple-support-level saturation anchors, blocked-time ridge lambda CV, repeated time-window placebos with BH warnings, and heterogeneity-aware multi-treated-market estimates. Fixed the `data.table::shift` import, direct workflow source-path handling, and executable package smoke coverage. TreeSHAP/Owen is deferred as a noncausal diagnostic because it cannot repair weak identification. Targeted Stan, transform, non-media, quasi-geo, handoff, and package smoke tests pass.

- `rolling_latest_reach_frequency_experiment_calibration`
  - Added a first-class Meridian-style RF path to package and standalone Stan workflows: population-scaled linear reach, Hill saturation over average frequency, effective-media adstock, and training-only normalization. The same transform drives fixed curves, sampled curves, warm starts, decomposition, response/scenario contribution, and fixed-impressions frequency planning. Added joint external-experiment calibration to `fit_hier_mmm(calibration_input=...)`; raw lift, ROI plus incremental spend, or cost-per-KPI plus incremental spend enters Stan on raw KPI scale with required SD/precision and posterior audit. Validation: RF/calibration contract passed; transform suite 23/23; Stan contract 50/50; standalone core 5/5; Stan compiled; RF/calibration HMC smoke completed; `R CMD check` completed with 0 errors, 0 warnings, and only the expected source-directory NOTE. Native ROI/mROI parameterization remains deferred, especially as a sampled-curve mROI sensitivity improvement.

- `rolling_latest_core_context_effect_modifiers`
  - Current rolling state after hardening opt-in context-varying effectiveness modifiers directly in the Stan workflow. Analysts can define context effects in metadata with tuple syntax such as `(context_col, 0.02, 0.03, +)`, or pass a `context_effects` table to `fit_hier_mmm()` / `prepare_stan_data_hier_mmm()`. Context drivers are standardized on training rows only; positive/negative/free signs map to constrained Stan parameter blocks; and the same multiplier is used by fitted transforms, decomposition, ROI/mROI, and optimizer contribution helpers. Plain `time` context is blocked by default because it can behave like drift; self-context is blocked; and outputs include context risk fields such as modeled-variable interaction flags, target-support correlation, prior multiplier range, posterior multiplier range, and review flags. This remains off by default and is intended for named hypotheses, not free time-varying coefficients. Validated with Stan source, workflow source, Stan compile, 41 Stan contract checks, 23 transform checks, 39 optimizer checks, and a 7-check known-truth context smoke test where TV context raised search effectiveness and macro context raised price sensitivity magnitude. The price-context test recovered direction but overestimated effect size, so context modifiers should be reviewed through the prior/posterior context audit rather than treated as automatic truth.
- `rolling_latest_core_keyed_coefficient_hierarchy`
  - Current rolling state after promoting `coef_hierarchy_scope = "keyed"` from metadata-only to an active single-level Stan coefficient pooling option. Analysts can use arbitrary composite `group_col` / model IDs such as `region_retailer_product` and pass one-time `coef_hierarchy_part_indices = 3` to pool within product, or `coef_hierarchy_part_indices = c(1, 3)` to pool within region-product. Metadata `hierarchy_part_indices` can override the global key for a specific variable. Indices are R-style one-based by default, with an optional zero-based escape hatch. `auto` hierarchy is now media/reach-frequency treatment-only by default, while controls/base/non-media variables require explicit `global` or `keyed` opt-in. The implementation keeps curve parameters shared at variable level, keeps `source_entity` as source/halo metadata rather than a hierarchy key, collapses a one-family keyed map to the regular global hierarchy to avoid an unnecessary extra latent layer, and leaves true multi-level nested/crossed hierarchy as future work. Validated with Stan source, 46 Stan contract checks, Stan compile/context smoke, and a tiny active-keyed Stan fit smoke.
...
- `rolling_latest_core_hierarchy_bau_rrate`
  - Current rolling state after adding guarded BAU rrate/adstock diagnostics and Stan hierarchy metadata guardrails. BAU now estimates one shared diagnostic rrate per variable only when a dependent/KPI column is supplied, preserves supplied rrate by default, and reports rrate status/improvement/bound flags in `rrate_diagnostics` and `curve_metadata`. Stan keeps curve parameters shared at variable level across groups while supporting `coef_hierarchy_scope = auto/global/none/keyed`, `hierarchy_key`, `model_id_parts`, and `hierarchy_part_indices` audit fields for coefficient hierarchy decisions. `none` disables group coefficient hierarchy, `global` preserves all-group pooling behavior, and `keyed` now supports active single-level keyed pooling when group/model IDs can be mapped to pooling families. Validated with BAU source/tests, Stan source/contract tests, and full workflow source.
