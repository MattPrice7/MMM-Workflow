...
- [ ] Refine evidence classification so synthetic-control failure is a downgrade only when TBR/DiD also fail or diagnostics are too weak.
...
- [ ] Add national time-series/TBR fallback for all-market media shocks, clearly labeled as lower-tier time-series evidence rather than geo-lift calibration.
...
- [ ] Add support-input planning with CPP/CPM/cost-to-support conversion where spend is not the execution variable.
- [ ] Add posterior uncertainty when posterior draw response curves are available.
- [ ] Add grouped/product-level constraints and rollups.
- [ ] Add optimizer outputs into deck builder charts.
- [ ] Add tests for zero spend, locked channels, infeasible constraints, negative contributions, missing spend, and grouped/product-level scenarios.
...
- [ ] Split outputs into client-facing, appendix, and internal QA chart registries.
- [ ] Fix chart date parsing, including Excel serial dates with origin `1899-12-30`.
- [ ] Add guards for missing fit columns and graceful chart skipping.
- [ ] Add client color palette/channel color inputs.
- [ ] Add response-curve charts with current spend/support markers and spend-percent scenarios.
- [ ] Add marginal ROI / marginal CPKPI curve charts.
- [ ] Add saturation/headroom summary charts.
